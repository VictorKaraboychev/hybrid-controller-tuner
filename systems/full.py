...
class FullSystem(System):
    ...
    def cost(self) -> float:
        ...
        control_effort_penalty_outer = max(
            0.0, metrics_outer["max_control_effort"] - 0.7
        )
        control_effort_penalty_inner = max(
            0.0, metrics_inner["max_control_effort"] - 6.0
        )
        ...
        settling_time_penalty_outer = max(
            0.0, metrics_outer["settling_time_2pct"] - 5.0
        )
        settling_time_penalty_inner = max(
            0.0, metrics_inner["settling_time_2pct"] - 0.25
        )
