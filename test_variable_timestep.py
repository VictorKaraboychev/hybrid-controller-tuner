...
def test_timestep_performance(system_class, system_params, r_func, t_end, dt, 
                             adaptive_tolerance=0.1, max_dt=0.1, name="System"):
    """
    ...
    start = time.time()
    results_fixed = simulate_system(
        system_fixed, r_func, t_end, dt=dt, dt_mode='fixed'
    )
    ...
    results_var = simulate_system(
        system_var, r_func, t_end, dt=dt, dt_mode='variable',
        adaptive_tolerance=adaptive_tolerance, max_dt=max_dt
    )
    ...
    print(f"{'-'*75}")
    print(f"{'Time (ms)':<30} {time_fixed*1000:<15.2f} {time_var*1000:<15.2f} "
          f"{(time_var-time_fixed)*1000:<15.2f}")
    print(f"{'Number of points':<30} {n_fixed:<15} {n_var:<15} "
          f"{n_var-n_fixed:<15}")
    print(f"{'Points/sec':<30} {n_fixed/time_fixed:<15.0f} {n_var/time_var:<15.0f} "
...
def main():
    ...
    result4 = test_timestep_performance(
        FullSystem,
        np.array([-1.920872,0.009013,-3.117162,0.394065,-7.892344,-0.998973,-0.177016,0.014990]),
        Step(amplitude=0.15),
        t_end=5.0,
        dt=0.002,
        adaptive_tolerance=0.1,
    ...
    print("="*60)
    print(f"{'Test':<30} {'Speedup':<15} {'Point Reduction':<15} {'Time Saved (ms)':<15}")
    print(f"{'-'*75}")
    ...
    for r in results:
        ...
        print(f"{r['name']:<30} {r['speedup']:<15.2f}x {r['point_reduction']:<15.1f}% "
    ...
    print(f"{'-'*75}")
    print(f"{'TOTAL':<30} {overall_speedup:<15.2f}x {'N/A':<15} {total_time_saved:<15.2f}")
    ...
    with open(output_file, 'w') as f:
        ...
        for r in results:
            ...
            f.write(f"  Variable: {r['n_var']} points in {r['time_var']*1000:.2f}ms\n\n")
